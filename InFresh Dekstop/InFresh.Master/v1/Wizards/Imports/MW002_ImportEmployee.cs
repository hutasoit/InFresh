...
{
    public partial class MW002_ImportEmployee : Form
    {
        #region Contructors and Properties
        /// <summary>
        /// 
        /// </summary>
        public MW002_ImportEmployee()
        {
            InitializeComponent();
        ...
        /// 
        /// </summary>
        protected const string Entity = "EMPL";
        /// <summary>
        /// 
        /// </summary>
        ...
        /// 
        /// </summary>
        protected IList<EmployeeDto> ExistEmployees { get; set; }
        /// <summary>
        /// 
        /// </summary>
        ...
        private bool MappingValid()
        {
                        ...
                        cmbAdd1Field.SelectedIndex == 0)
            {
                MessageBox.Show(null,
                    string.Format(MasterModule.Handler.Resources.GetString("Fields_Required"),
                        string.Format("- {0} \n -{1}", "Employee Name", "Employee Address")),
                    MasterModule.Name,
            ...
            {
                MessageBox.Show(null,
                    string.Format(MasterModule.Handler.Resources.GetString("Field_Required"), "Employee Name"),
                    MasterModule.Name,
                    MessageBoxButtons.OK,
                    MessageBoxIcon.Warning);
                cmbNameField.Focus();
            ...
            {
                MessageBox.Show(null,
                    string.Format(MasterModule.Handler.Resources.GetString("Field_Required"), "Employee Address"),
                    MasterModule.Name,
                    MessageBoxButtons.OK,
                    MessageBoxIcon.Warning);
                cmbAdd1Field.Focus();
        ...
        private bool MappingValue()
        {
            ...
            try
            {
                int count = ExistEmployees.Count();
                EmployeeDto mm = null;
                int ii = 0;
                foreach (var row in rows)
                {
                    ...
                    if (cmbNameField.SelectedIndex != 0)
                    {
                        ...
                        {
                            mm.Name = mm.Code;
                            errMsg += "Error Name Employee on line " + (ii + 1) + "\n";
                        }
                    }
                    if (cmbAdd1Field.SelectedIndex != 0)
                    {
                        ...
                        {
                            mm.Address1 = mm.Code;
                            errMsg += "Error Address Employee on line " + (ii + 1) + "\n";
                        }
                    }
                    ...
                    if (cmbPhone2Field.SelectedIndex != 0)
                    {
                        if (row.Field<object>(cmbPhone2Field.SelectedValue.ToString()) != null)
                            mm.Phone2 = row.Field<object>(cmbPhone2Field.SelectedValue.ToString()).ToString();
                        else
                            mm.Phone2 = string.Empty;
                    }
            ...
            if (cmbTemplate.SelectedIndex == 0)
            {
                ...
                if (cmbPhone2Field.SelectedIndex != 0)
                    FieldsTemplate.Add(new Template2Dto()
                    {
                        Code = code,
                        Sequence =  9,
                        Source = cmbPhone2Field.SelectedValue.ToString(),
                        Destination = "EMPHN2"
                    });
                else
                    FieldsTemplate.Add(new Template2Dto()
        ...
        private void ReadTemplate()
        {
            if (cmbTemplate.SelectedIndex != 0)
            {
                ...
                {
                    var header = Template.FieldsTemplate.Where(m => m.Destination.Equals("EMEMNM"))
                    ...
                    header = Template.FieldsTemplate.Where(m => m.Destination.Equals("EMEMON"))
                                    .Select(m => m.Source).FirstOrDefault();
                    if (!string.IsNullOrWhiteSpace(header))
                    {
                        if (HeaderFile.Contains(header))
                    ...
                    header = Template.FieldsTemplate.Where(m => m.Destination.Equals("EMADD1"))
                                    .Select(m => m.Source).FirstOrDefault();
                    if (!string.IsNullOrWhiteSpace(header))
                    {
                        if (HeaderFile.Contains(header))
                    ...
                    header = Template.FieldsTemplate.Where(m => m.Destination.Equals("EMADD2"))
                                    .Select(m => m.Source).FirstOrDefault();
                    if (!string.IsNullOrWhiteSpace(header))
                    {
                        if (HeaderFile.Contains(header))
                    ...
                    header = Template.FieldsTemplate.Where(m => m.Destination.Equals("EMCITY"))
                                    .Select(m => m.Source).FirstOrDefault();
                    if (!string.IsNullOrWhiteSpace(header))
                    {
                        if (HeaderFile.Contains(header))
                    ...
                    header = Template.FieldsTemplate.Where(m => m.Destination.Equals("EMZPCD"))
                                    .Select(m => m.Source).FirstOrDefault();
                    if (!string.IsNullOrWhiteSpace(header))
                    {
                        if (HeaderFile.Contains(header))
                    ...
                    header = Template.FieldsTemplate.Where(m => m.Destination.Equals("EMPHN1"))
                                    .Select(m => m.Source).FirstOrDefault();
                    if (!string.IsNullOrWhiteSpace(header))
                    {
                        if (HeaderFile.Contains(header))
                    ...
                    header = Template.FieldsTemplate.Where(m => m.Destination.Equals("EMPHN2"))
                                    .Select(m => m.Source).FirstOrDefault();
                    if (!string.IsNullOrWhiteSpace(header))
                    {
                        if (HeaderFile.Contains(header))
                            cmbPhone2Field.Text = header;
                        else
                            cmbPhone2Field.SelectedIndex = 0;
                    }
                    else
                        cmbPhone2Field.SelectedIndex = 0;
                    cmbPhone2Field.Enabled = false;

                    header = Template.FieldsTemplate.Where(m => m.Destination.Equals("EMEMIL"))
                                    .Select(m => m.Source).FirstOrDefault();
                    if (!string.IsNullOrWhiteSpace(header))
                    {
                        if (HeaderFile.Contains(header))
